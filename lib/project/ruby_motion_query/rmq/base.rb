module RubyMotionQuery
  ...
  class RMQ
    ...
    def context=(value)
      if value
        if value.is_a?(NSWindowController)
          @context = RubyMotionQuery::RMQ.weak_ref(value)
        elsif value.is_a?(NSView)
          @context = value
        #else
    ...
    # The NSViews currently selected
    #
    # Use {#get} instead to get the actual NSView objects
    #
    # @return [Array]
    def selected
      if @selected_dirty
    ...
    def context_or_context_view
      if @context.is_a?(NSWindowController)
        @context.window.contentView
      else
        @context
      end
    end
    ...
    def all_superviews_for(view, out = [])
      if (nr = view.nextResponder) && nr.is_a?(NSView)
        out << nr
